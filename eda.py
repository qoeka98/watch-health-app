...
def run_eda():
    ...
    if submit:
        ...
        # ▶️ 라이프스타일 보정 (모든 질병에 적용)
        # 흡연 시 +10, 음주 시 +10, 운동 시 -15 (고정 포인트 추가)
        lifestyle_adjustment_points = 0
        if smoke:
            lifestyle_adjustment_points += 10
        if alco:
            lifestyle_adjustment_points += 10
        if active:
            lifestyle_adjustment_points -= 15
        
        for disease in disease_probabilities:
            adjusted = disease_probabilities[disease] + lifestyle_adjustment_points
