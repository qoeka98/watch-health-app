...
def calculate_hypertension_risk(systolic_bp, diastolic_bp, smoke, alco, active):
    """
    고혈압 위험도를 혈압 수치에 기반하여 직접 계산하는 함수.
    """
    base_risk = 0
    if systolic_bp >= 140 or diastolic_bp >= 90:
        base_risk = 80  # 고혈압 기준을 초과할 경우 높은 기본 위험도
    elif systolic_bp >= 130 or diastolic_bp >= 85:
        base_risk = 60  # 경계성 고혈압
    elif systolic_bp >= 120 or diastolic_bp >= 80:
        base_risk = 40  # 정상 고혈압 범위
    else:
        base_risk = 20  # 정상 범위
    
    # 라이프스타일 보정 (보다 합리적으로 조정)
    if smoke == 0:   # 흡연하면 위험도 증가
        base_risk += 10
    if alco == 0:    # 음주하면 위험도 증가
        base_risk += 10
    if active == 0:  # 운동하면 위험도 감소
        base_risk -= 10
    
    return min(max(base_risk, 0), 100)  # 위험도를 0~100 사이로 제한
