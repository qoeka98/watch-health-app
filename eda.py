...
def calculate_bmi(weight, height):
    if height > 0:
        ...
        if bmi < 18.5:
            ...
            category = "과체중 경고"
            min_obesity_risk = 50  # ✅ 과체중일 경우 최소 50% 보장
        elif 25 <= bmi < 27:
            category = "과체중"
            min_obesity_risk = 60  # ✅ BMI 25 이상이면 최소 60% 보장
        elif 27 <= bmi < 30:
            category = "경도 비만"
            min_obesity_risk = 70  # ✅ BMI 27 이상이면 최소 70% 보장
        elif 30 <= bmi < 35:
            category = "중등도 비만"
            min_obesity_risk = 80  # ✅ BMI 30 이상이면 최소 80% 보장
        else:
            category = "고도 비만"
            min_obesity_risk = 90  # ✅ BMI 35 이상이면 최소 90% 보장
...
# ✅ 질병 확률 보정 함수 (BMI 기반 비만 확률 보정 추가)
def adjust_probabilities(probabilities, bmi_risk):
    probabilities["비만"] = max(probabilities["비만"], bmi_risk)  # ✅ BMI 기반 최소 비만 확률 적용
