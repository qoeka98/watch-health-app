...
def run_eda():
    ...
    # 예측 확률 보정
    for disease in disease_probabilities:
        disease_probabilities[disease] = sigmoid_scaling(disease_probabilities[disease] / 100.0) * 1000

    print("📌 보정된 예측 확률:", disease_probabilities)

        # 🔹 Min-Max Scaling (0~100% 범위로 정규화)
    def min_max_scale(value, min_val=0.01, max_val=0.6):  # 기존 0.5 → 0.6 조정하여 확률 분포 개선
