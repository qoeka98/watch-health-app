...
def run_eda():
    ...
    if submit:
        ...
        # (7) 나이 보정 적용
        # 기준 나이 50세를 사용하되, 70세 이상은 효과가 고정되도록
        effective_age = age if age <= 70 else 70
        for disease in disease_probabilities:
            if disease == "고혈압":
                # 고혈압은 나이 보정 효과를 0.5배로 적용
        ...
        # --- 5. 평균 비교 차트 (나이, 키 제외; 몸무게 옆에 사용자 BMI 표시) ---
        st.markdown("---")
